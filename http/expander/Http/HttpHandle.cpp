...
static void changeDeviceIP(std::string ip, std::string gw, std::string dns)
{
    ...
    if (fin.is_open())
    {
        while ( getline(fin, line) )
        {
            ...
            fout << line << std::endl;
            if (line == "#fallback static_eth0")
            {
                std::cout << "Changing ... " << std::endl;
                std::cout << fin.tellg() << std::endl;
                fout << std::endl;
                fout << "# Expander IP" << std::endl;
...
void apiNetworkInfo(struct evhttp_request *request)
{
    try
    {
        ...
        {
            json jInfo;
            // jInfo["IpMode"] = (getCmdResult("ip route | grep 'default' | grep 'dhcp' | cut -d' ' -f7") == "dhcp")? "static" : "dhcp";
            jInfo["IpMode"] = (getCmdResult("ip route | grep 'default' | grep 'dhcp' | cut -d' ' -f7") == "dhcp")? "dhcp" : "static";
...
void apiLogin(struct evhttp_request *request)
{
    try 
    {
        ...
        if (readMethod(request) == "POST")
        {
            ...
            jInfor = json::parse(loginInfo);
            if (jInfor.contains("username") && jInfor.contains("password"))
            {
                std::string userID = jInfor["username"];
                std::string pass = jInfor["password"];
                if (database->checkLoginInfo(userID, pass))
                {
                    std::string tokenGen = authenticate->tokenGenerate(userID, pass);
                    jRespond["status"] = true;
                    jRespond["message"] = "post.login.success";
                    jRespond["access_token"] = tokenGen;
                    sendJson(request, 200, jRespond);
            ...
            else if (jInfor.contains("new_password"))
            {
                std::string pass = jInfor["new_password"];
                database->createPassword(pass);
                jRespond["message"] = "Create password successfully";
                sendJson(request, 200, jRespond);
            }
            else
            {
        ...
        {
            jInfor["status"] = database->getPassword();
            if(jInfor["status"] == 1)
                sendJson(request, 200, jInfor);
        }
        else
        {
            sendError(request, 400, "Wrong request method.");
...
void apiIp(struct evhttp_request *request)
{
    try
    {
        if (readMethod(request) == "POST")
        {
            ...
            else if (jEvent.contains("mode"))
            {
                sendJson(request, 200, "change ip successfully.");
                std::system("sudo systemctl restart piserver.service");
                std::system("sudo cp /home/pi/Program/dhcp/temp.conf /etc/dhcpcd.conf");
                start_timer(4000, reboot, NULL);
            }
            else
            {
...
static int getToken(HttpClient *client, camera_t &camera)
{
    ...
    if (200 == ret)
    {
        try 
        {
            ...
            std::cout << "Get token successfully, Response " << camera.ip << std::endl;
        }
        catch(const std::exception& e)
        {
            std::cout << "Get access_token failed, please check again!" << std::endl;
            std::cerr << e.what() << '\n';
        }
        return ret;
    }
...
static int sendOpendoor(HttpClient *client, camera_t &camera, int channel, struct evhttp_request *request)
{
    json jContent;
    jContent["enable"] = true;
    jContent["url_server"] = getCmdResult("hostname -I | cut -d ' ' -f1") + ":55555/opendoor/" + std::to_string(channel);
    std::string response;
    std::string url = "https://" + camera.ip + ":4004/api/setup/opendoor?access_token=" + camera.token;
    int ret = client->request(url, POST, jContent.dump(), response, 10);
    ...
    if (ret == 200)
    {
        try 
        {
            std::cout << "Request Opendoor Success, respone:  " << response << std::endl;
        }
        catch(const std::exception& e)
        {
            std::cerr << e.what() << '\n';
    ...
    std::cout << "Request Opendoor Failed, respone: " << response << std::endl; 
    return 0;
}
                ...
                */
                switch (action)
                {
                    ...
                    case 4: // change camera ip
                    {
                            ...
                            jConfig["cameraip"].is_array())
                        {
                            ...
                            if (jOldCameraIp.size() < jConfig["cameraip"].size())
                            {
                                for (auto newCam : jConfig["cameraip"]){
                                    jNewCameraIp = newCam;
                                    std::cout << "jNewCameraIp: " << jNewCameraIp << std::endl;
                                }
                                newCameraIp.ip = jNewCameraIp["ip"];
                                newCameraIp.username = jNewCameraIp["username"];
                                newCameraIp.password = jNewCameraIp["password"];
                                int respones = getToken(client, newCameraIp);
                                if(200 == respones)
                                {
                                    sendOpendoor(client, newCameraIp, channel, request);
                                    syncprofile->putAllProfile(channel, newCameraIp);
                                }
                                else if(400 == respones)
                                {
                                    sendJson(request, 400, "The username or password is incorrect. Try again");
                                    break;
                                }
                                else
                                {
                                    sendJson(request, 400, "Couldn't connect to camera");
                                    break;
                                }
                            }
                        ...
                        {
                            sendError(request, 400, "Insert cameraip adasdasdasd");
                        }
                        break;
                    }
...
void apiDeviceConfig(struct evhttp_request *request)
{
    try
    {
        if (readMethod(request) == "POST")
        {
            ...
            auto jConfig = json::parse(content);
            int channel = jConfig["channel"];
            if (jConfig.contains("transfer"))
            {
                if (jConfig["transfer"] >= MODE_SERVER && jConfig["transfer"] <= MODE_PEER)
                {
                    database->changeTransfer(jConfig["channel"], jConfig["transfer"]);
                    sendJson(request, 200, "Change mode successfully");
                    logi->writeLog(
                        readIpClient(request),
                        readMethod(request),
                        "/api/deviceconfig",
                        "CHANGE mode = " + std::to_string((int)jConfig["transfer"]) + ", channel = " + std::to_string(channel)
                    );    
                }
                else
                {
            ...
            else if (jConfig.contains("mode"))
            {
                if (jConfig["mode"] >= MODE_ALL && jConfig["mode"] <= MODE_CLIENT)
                {
                    database->changeUser(jConfig["channel"], jConfig["mode"]);
                    sendJson(request, 200, "Change mode successfully");
                    logi->writeLog(
                        readIpClient(request),
                        readMethod(request),
                        "/api/deviceconfig",
                        "CHANGE mode = " + std::to_string((int)jConfig["mode"]) + ", channel = " + std::to_string(channel)
                    );    
                }
                else
                {
            ...
            else if (jConfig.contains("action"))
            {
                json jGetMode;
                int gTransfer = database->getTransfer(channel);
                int gUser = database->getUser(channel);
                jGetMode["gTransfer"] = gTransfer;
                jGetMode["gUser"] = gUser;
                sendJson(request, 200, jGetMode);
                logi->writeLog(
                    readIpClient(request),
                    readMethod(request),
                    "/api/deviceconfig",
                    "GET transfer, channel = " + std::to_string(channel) + ", gTransfer = " + std::to_string(gTransfer) + ", gUser = " + std::to_string(gUser)
                );
            }
            else
            {
...
void cameraOpendoorHandle(struct evhttp_request *request, int channel)
{
    try
    {
        ...
        // syncprofile->mode = database->getAllTransfer();
        json jIDArray = json::array();
        json jContent = json::parse(content);

        switch (database->getTypeEvent(channel))
        {
        case FACE_RECOGNITION:
            type = "facerecognition";
            break;
        case LICENSE_RECOGNITION:
            type = "licenserecognition";
            break;
        case INTRUSION_DETECTION:
            type = "intrusion"; 
            break;
        }
        bool b_FR = false, b_Ins = false;

        if (!jContent.contains("type")) SEND_ERROR_RETURN(request, 400, "type not found");

        if (jContent["type"] == "facerecognition" && type == "facerecognition")
        {
        ...
        else if (jContent["type"] == "intrusion" && type == "intrusion")
        {
            std::cout << "intrusion" << std::endl;
            if (!jContent.contains("motion_status")) SEND_ERROR_RETURN(request, 400, "Type: intrusion, motion_status not found");
            jIDArray = jContent["motion_status"];
            b_Ins = true;
        }
        ...
        if(b_FR)
        {
            ...
            for (std::string id : jIDArray)
            {
                ...
                {
                    sendJson(request, 400, "wrong method");
                }
            ...
            sendJson(request, 200, "Accepted ID: " + ((IDAccept == "") ? "None" : IDAccept));
            logi->writeLog(readIpClient(request), readMethod(request),"/opendoor/" + std::to_string(channel),
                "Accepted ID: " + ((IDAccept == "") ? "None" : IDAccept));
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            std::cout << "time opendoor: " << duration.count()/1e6 << " s" << std::endl;
        }
        else if(b_Ins)
        {
            control->ControlOpenDeviceDelay(channel, database->getTimeOpen(channel));
            control->ControlOpenDeviceDelay(3, database->getTimeOpen(channel));
            sendJson(request, 200, "Warning !!!!");
            logi->writeLog(readIpClient(request), readMethod(request),"/opendoor/" + std::to_string(channel),
                "Warning !!!!");
        }
        else
        {
            sendJson(request, 400, "Bad !!!!");
...
void cameraEventHandle(struct evhttp_request *request)
{
    try
    {
        ...
        for (json object : j["extras"]["objects"])
        {
            ...
            else if (jEvent["type"] == "intrusion")  jEvent["content"] = object["motion_status"];
