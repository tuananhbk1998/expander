...
int Database::getPassword()
{
    int ret;
    int error = 0;
    sqlite3_stmt *stmt;
    std::string cmd = "SELECT PASSWORD from USER WHERE USERID = 'admin'";
    error = sqlite3_prepare_v2(db, cmd.c_str(), -1, &stmt, NULL);
    if (error != SQLITE_OK){
        ret = 0;
    }
    while (sqlite3_step(stmt) != SQLITE_DONE) {
        if(std::string((const char*) (sqlite3_column_text(stmt,0))).empty())
            ret = 1;
        else 
            ret = 0;
    }
...
int Database::createPassword(std::string password)
{
    int ret = 0;
    char *errMsg = NULL;
    // std::string cmd = "UPDATE USER set PASSWORD = '" + password + "' where USERID = 'admin';";
    std::string cmd = "UPDATE USER set PASSWORD = '" + password + "';";
    ret = sqlite3_exec(db, cmd.c_str(), NULL, NULL, &errMsg);
    if (ret != SQLITE_OK)
        std::cout << "Create pasword SQL error: " << errMsg << std::endl;
    sqlite3_free(errMsg);
    return (ret == SQLITE_OK);
}
...
void Database::createDeviceConfig()
{
    ...
    std::string sql = "INSERT INTO DEVICE_CONFIG_TABLE (CHANNEL, TRANSFER, USER, PARAM) VALUES (1, 0, 0, 'MODE');";
    ret = sqlite3_exec(db, sql.c_str(), NULL, NULL, &errMsg);

    sql = "INSERT INTO DEVICE_CONFIG_TABLE (CHANNEL, TRANSFER, USER, PARAM) VALUES (2, 0, 0, 'MODE');";
    ret = sqlite3_exec(db, sql.c_str(), NULL, NULL, &errMsg);

    sql = "INSERT INTO DEVICE_CONFIG_TABLE (CHANNEL, TRANSFER, USER, PARAM) VALUES (3, 0, 0, 'MODE');";
    ret = sqlite3_exec(db, sql.c_str(), NULL, NULL, &errMsg);
...
int Database::changeTransfer(int channel, int mode)
{
    int ret = 0;
    char *errMsg = NULL;
    std::string cmd = "UPDATE DEVICE_CONFIG_TABLE set TRANSFER = " + std::to_string(mode) + " where CHANNEL = " + std::to_string(channel) + ";";
    ret = sqlite3_exec(db, cmd.c_str(), NULL, NULL, &errMsg);
    if (ret != SQLITE_OK)
        std::cout << "changeTransfer SQL error: " << errMsg << std::endl;
    else
        std::cout << "changeTransfer, value: " << mode << " OK" << std::endl;

    sqlite3_free(errMsg);
    return (ret == SQLITE_OK);
}

int Database::getTransfer(int channel)
{
    int syncMode = 0;
    int error = 0;
    sqlite3_stmt *stmt;
    std::string cmd = "SELECT TRANSFER from DEVICE_CONFIG_TABLE where CHANNEL = " + std::to_string(channel) + ";";
    error = sqlite3_prepare_v2(db, cmd.c_str(), -1, &stmt, NULL);
    if (error != SQLITE_OK){
        std::cout << "getTransfer error" << std::endl;
        return -1;
    }
    while (sqlite3_step(stmt) != SQLITE_DONE) {
...
int Database::changeUser(int channel, int mode)
{
    int ret = 0;
    char *errMsg = NULL;
    std::string cmd = "UPDATE DEVICE_CONFIG_TABLE set USER = " + std::to_string(mode) + " where CHANNEL = " + std::to_string(channel) + ";";
    ret = sqlite3_exec(db, cmd.c_str(), NULL, NULL, &errMsg);
    if (ret != SQLITE_OK)
...
int Database::getUser(int channel)
{
    int syncUser = 0;
    int error = 0;
    sqlite3_stmt *stmt;
    std::string cmd = "SELECT USER from DEVICE_CONFIG_TABLE where CHANNEL = " + std::to_string(channel) + ";";
    error = sqlite3_prepare_v2(db, cmd.c_str(), -1, &stmt, NULL);
    if (error != SQLITE_OK){
        std::cout << "getUser error" << std::endl;
        return -1;
    }
    while (sqlite3_step(stmt) != SQLITE_DONE) {
        syncUser = stoi(std::string((const char*) (sqlite3_column_text(stmt,0))));
    }
    sqlite3_finalize(stmt);
    return syncUser;
}

int Database::getAllTransfer()
{
    int getAllTransfer = 0;
    int error = 0;
    sqlite3_stmt *stmt;
    std::string cmd = "SELECT TRANSFER from DEVICE_CONFIG_TABLE where PARAM = 'MODE';";
    error = sqlite3_prepare_v2(db, cmd.c_str(), -1, &stmt, NULL);
    if (error != SQLITE_OK){
        std::cout << "getAllTransfer error" << std::endl;
        return -1;
    }
    while (sqlite3_step(stmt) != SQLITE_DONE) {
        getAllTransfer = stoi(std::string((const char*) (sqlite3_column_text(stmt,0))));
    }
    sqlite3_finalize(stmt);
    return getAllTransfer;
}

int Database::getAllUser()
{
    int getAllUser = 0;
    int error = 0;
    sqlite3_stmt *stmt;
    std::string cmd = "SELECT USER from DEVICE_CONFIG_TABLE where PARAM = 'MODE';";
    error = sqlite3_prepare_v2(db, cmd.c_str(), -1, &stmt, NULL);
    if (error != SQLITE_OK){
        std::cout << "getAllUser error" << std::endl;
        return -1;
    }
    while (sqlite3_step(stmt) != SQLITE_DONE) {
        getAllUser = stoi(std::string((const char*) (sqlite3_column_text(stmt,0))));
    }
    sqlite3_finalize(stmt);
    return getAllUser;
}