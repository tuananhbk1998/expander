...
public: 
    ...
    int getPassword();
    int createPassword(std::string password);
    ...
    /* DEVICE CONFIG*/
    int changeTransfer(int channel, int syncMode);
    int getTransfer(int channel);
    int changeUser(int channel, int syncMode);
    int getUser(int channel);
    int getAllTransfer();
    int getAllUser();
private:
    sqlite3 *db;
